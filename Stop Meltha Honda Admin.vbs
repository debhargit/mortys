...
If Not fso.FileExists(nodeExe) Then
    MsgBox "runtime\node.exe is missing - nothing to stop.", vbCritical, "Meltha Honda Admin"
    WScript.Quit 1
End If
...
answer = MsgBox("Stop the Meltha Honda admin server?" & vbCrLf & vbCrLf & _
                "Anyone else on the network using the admin panel or POS on this " & _
                "machine will be disconnected immediately.", _
                vbQuestion + vbYesNo + vbDefaultButton2, "Meltha Honda Admin")
...
MsgBox "Meltha Honda admin server stopped.", vbInformation, "Meltha Honda Admin"
WScript.Quit 0
